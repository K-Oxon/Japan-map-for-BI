load spatial;
create or replace table dwh.dwh.ja_admnstrtv_area_simplify_1741 as (
    with raw_area as (
      select *
      from dwh.dwh.ja_admnstrtv_area_simplify
...
select count(distinct lg_code) as unq_cnt_lg_code from dwh.dwh.ja_admnstrtv_area_simplify_1741;
...
COPY (
  ...
  order by lg_code
) TO 'processing/intermediate_data/transformed/ja_municipality_area_simplified_1741.geojson' WITH (
  FORMAT GDAL,
  DRIVER 'GeoJSON',
